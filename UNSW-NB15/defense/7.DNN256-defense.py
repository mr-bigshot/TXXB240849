...
from utils_new import addattack, add_repaired
...
dataset_train_data = torch.from_numpy(dataset_train[:,0:42]).float()
dataset_train_label = torch.from_numpy(dataset_train[:,42:43]).long()
dataset_test_data = torch.from_numpy(dataset_test[:,0:42]).float()
dataset_test_label = torch.from_numpy(dataset_test[:,42:43]).long()
...
PATH = "model_state_dict_DNN_256.pth"
model_load = net
...
dataset_attack_data = torch.from_numpy(dataset_attack_np[:,0:42]).float()
dataset_attack_label = torch.from_numpy(dataset_attack_np[:,42:43]).long()
...
dataset_repaired_data = torch.from_numpy(dataset_repaired[:, 0:42]).float()
dataset_repaired_label = torch.from_numpy(dataset_repaired[:, 42:43]).long()
...
# torch.save(net.state_dict(), "model_state_dict_DNN256_repaired.pth")

PATH = "model_state_dict_DNN256_repaired.pth"
model_repaired = net
